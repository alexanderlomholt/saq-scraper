task :saq_store_scraper => :environment do
  ...
  html_doc.search('.fiche').each do |element|
    ...
    name = element.css('.entete .titre a').inner_html[0..-6].strip
    address = element.css('.adresse').text.strip.gsub(/\s+/," ").gsub(/(\(|\)|\#)/,"").unicode_normalize(:nfkd).encode('ASCII', replace: '')
    phoneNumber = element.css('.tel').text.strip[16..27]

    # api call -> Google geocoder
    ...
    # store info in new Store db instances
    Store.create(
